...
func TestPad16(t *testing.T){
	...
	},
	{
		...
		expected: &image.Gray16{
			...
			Pix: []uint8{
				...
				0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
				0x40, 0x40, 0x40, 0x40, 0x40, 0xFF, 0x40, 0xFF,
				0x40, 0x40, 0x40, 0x40, 0x40, 0xFF, 0x40, 0xFF,
			},
		},
	},
	...
	for _, c := range cases {
		...
		actual := PadG16(c.value, c.x, c.y, c.method)

		if !util.Gray16ImageEqual(actual, c.expected) {
			t.Errorf("%s:\nexpected:%v\nactual:%v", "Pad "+c.desc, util.Gray16ToString(c.expected), util.Gray16ToString(actual))
		}
	}
}
